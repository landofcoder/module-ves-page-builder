...
class AbstractWidget extends \Magento\Framework\View\Element\Template implements \Magento\Widget\Block\BlockInterface
{
    ...
    public function getColStyle($col = array())
    {
        ...
        if (isset($row->bgcolor) && $row->bgcolor) {
            $custom_col_css[] = 'background-color:' . $row->bgcolor;
        }
        if (isset($row->bgimage) && $row->bgimage) {
            $row->bgimage = $this->getImageUrl($row->bgimage);
            $custom_col_css[] = $row->bgimage ? 'background-image:url(' . $row->bgimage . ')' : '';
        }
        if (isset($row->bgrepeat) && $row->bgrepeat) {
            $custom_col_css[] = 'background-repeat:' . $row->bgrepeat;
        }
        if (isset($row->bgposition) && $row->bgposition) {
            $custom_col_css[] = 'background-position:' . $row->bgposition;
        }
        if (isset($row->bgattachment) && $row->bgattachment) {
            $custom_col_css[] = 'background-attachment:' . $row->bgattachment;
        }
        if (isset($row->padding) && $row->padding) {
            $custom_col_css[] = 'padding:' . $row->padding;
        }
        if (isset($row->padding) && $row->padding) {
            $custom_col_css[] = 'margin:' . $row->padding;
        }
    ...
    public function getWidgetStyle($col = array())
    {
        ...
        if (isset($row->bgcolor) && $row->bgcolor) {
            $custom_widget_css[] = 'background-color:' . $row->bgcolor;
        }
        if (isset($row->bgimage) && $row->bgimage) {
            $row->bgimage = $this->getImageUrl($row->bgimage);
            $custom_widget_css[] = $row->bgimage ? 'background-image:url(' . $row->bgimage . ')' : '';
        }
        if (isset($row->bgrepeat) && $row->bgrepeat) {
            $custom_widget_css[] = 'background-repeat:' . $row->bgrepeat;
        }
        if (isset($row->bgposition) && $row->bgposition) {
            $custom_widget_css[] = 'background-position:' . $row->bgposition;
        }
        if (isset($row->bgattachment) && $row->bgattachment) {
            $custom_widget_css[] = 'background-attachment:' . $row->bgattachment;
        }
        if (isset($row->padding) && $row->padding) {
            $custom_widget_css[] = 'padding:' . $row->padding;
        }
        if (isset($row->padding) && $row->padding) {
            $custom_widget_css[] = 'margin:' . $row->padding;
        }
