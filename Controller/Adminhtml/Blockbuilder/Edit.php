...
class Edit extends \Magento\Backend\App\Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('Ves_PageBuilder::pages_builder')
            ...
            ->addBreadcrumb(__('Manage Element Profiles'), __('Manage Element Profiles'));
        return $resultPage;
    }
    ...
    public function execute()
    {
    	...
    	if($id){
    		$model->load($id);
    		if(!$model->getId()){
    			$this->messageManager->addError(__('This element builder profile no longer exits.'));
        ...
        $resultPage->addBreadcrumb(
            $id ? __('Edit Element Profile') : __('New Element Profile'),
            $id ? __('Edit Element Profile') : __('New Element Profile')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Pages Builder'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Element Profile'));
